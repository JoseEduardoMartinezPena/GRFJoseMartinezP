...
public class Camcap extends javax.swing.JFrame {
    ...
    CascadeClassifier pickDetector;
    ...
    MatOfRect pickDetections;

    String File_path = "";
    String base_path = "C:/Proyecto OpenCV/";
        //"./data/haarcascades/";   // Carpeta donde se localizan los modelos de reconocimiento
    //String faceFile = "cascade.xml";
        //"haarcascade_frontalface_alt.xml";  // Archivo referencia para reconocimiento de rostros
    //String eyeFile = "haarcascade_eye.xml";  // Archivo referencia para reconocimiento de rostros
    String pickFile = "cascade.xml";
    ...
    class DaemonThread implements Runnable {
        ...
        public void run() {
            ...
            pickDetector = new CascadeClassifier(base_path + pickFile);
            pickDetections = new MatOfRect();
                ...
                while (runnable) {
                    if (webSource.grab()) {
                        try {
                            ...
                            pickDetector.detectMultiScale(frame, pickDetections);

                            int i = 0;

                            for (Rect rect : pickDetections.toArray()) {

                                Imgproc.rectangle(frame, new Point(rect.x, rect.y), new Point(rect.x + rect.width, rect.y + rect.height),
                                        new Scalar(0, 0, 255));

                                pua = frame.submat(pickDetections.toArray()[i]);

                                System.out.println("Se encontraron " + pickDetections.toArray().length + " pua(s)");
                            }
